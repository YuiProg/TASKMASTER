...
public class AuthController implements UserProcessService {
    ...
    @Override
    @PostMapping("/api/v1/confirmResetPasswordCode/{code}")
    public ResponseEntity<ApiResponseModel<String>> confirmResetPasswordCode(
            @RequestBody UserRequest userRequest,
            @PathVariable String code
    ) {
        return userProcessService.confirmResetPasswordCode(userRequest, code);
    }

    @Override
    @PostMapping("/api/v1/emailUsers")
    public ResponseEntity<ApiResponseModel<String>> emailUsers() {
        return userProcessService.emailUsers();
    }
